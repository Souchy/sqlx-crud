...
fn build_sqlx_crud_impl(config: &Config) -> TokenStream2 {
    ...
    let insert_query_args = config
        ...
        .flat_map(|f| &f.ident)
        .filter(|i| config.external_id || *i != &config.id_column_ident)
        .map(|i| quote! { args.add(self.#i).map_err(sqlx::Error::Encode)?; });
    ...
    let update_query_args_id = quote! { args.add(self.#id_column_ident).map_err(sqlx::Error::Encode)?; };
    ...
    quote! {
        ...
        impl<'e> #crate_name::traits::Crud<'e, &'e ::sqlx::pool::Pool<#db_ty>> for #ident {
            fn insert_args(self) -> ::sqlx::Result<<#db_ty as ::sqlx::database::Database>::Arguments<'e>> {
            ...
            fn update_args(self) -> ::sqlx::Result<<#db_ty as ::sqlx::database::Database>::Arguments<'e>> {
                use ::sqlx::Arguments as _;
                let mut args = <#db_ty as ::sqlx::database::Database>::Arguments::default();
